...
function App() {
  ...
  const getHistory = () => {
    ...
    fetch(
      `https://api.thegraph.com/subgraphs/name/sotatek-tuannguyen8/web3-classs`,
      {
        method: "POST",
      ...
      .then((data) => {
        ...
        const arData = approvalEntities.concat(
          depositEntities,
          transferEntities,
          withdrawEntities
        );
  ...
  useEffect(() => {
    injected
      .isAuthorized()
      .then((isAuthorized) => {
        if (isAuthorized && !active && !network) {
          activate(injected);
        }
      })
      .catch(() => {});
  }, [active, activate, network]);
  ...
  return (
    ...
    >
      {!active ? (
        ...
        >
          <button style={{ height: 50, width: 200 }} onClick={connectMetamask}>
            Connect Metamask
          </button>
          <button style={{ height: 50, width: 200 }} onClick={connectWallet}>
        ...
        >
          ...
          >
            ...
            >
              ...
              <button onClick={() => deposit(library, account, "0")}>
                Harvest
              </button>
            </div>
            <div style={{ width: "100%" }}>
          ...
          <div>
            <table>
              <tr style={{ border: "1px solid black" }}>
                <th style={{ border: "1px solid black", padding: 30 }}>
                  Action
                </th>
                <th style={{ border: "1px solid black", padding: 30 }}>
                  Amout
              ...
              {dataHistory?.map((item, index) => (
                <tr key={index}>
                  <td style={{ border: "1px solid black", padding: 30 }}>
                    {item.type}
                  </td>
                  <td style={{ border: "1px solid black", padding: 30 }}>
                    {ethers.utils.formatEther(item.amout)}
                  </td>
                  <td style={{ border: "1px solid black", padding: 30 }}>
                    <span>
                      {moment(convertTimestamp(item.time).toString()).format(
                        "HH:mm:ss DD/MM/YYYY"
                      )}
                    </span>
                  </td>
      ...
      <Modal
        ...
        visible={modalStake}
        onOk={() => {
          deposit(library, account, stakeInputRef.current.value);
          setModalStake(false);
        }}
      ...
      >
        ...
        <div style={{ marginTop: 10 }}>
          Your WETH balance: {dataUser?.balance}
        </div>
      </Modal>
      <Modal
        title="Stake"
        ...
        onOk={() => {
          withdraw(library, account, stakeInputRef.current.value);
      ...
      >
        ...
        ></input>
        <div style={{ marginTop: 10 }}>
          Your WETH deposited: {dataUser?.isApproved}
        </div>
      </Modal>
    </div>
  );
